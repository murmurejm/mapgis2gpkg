...
def convert_folder(folder=None,
    ...
    if files:
        ...
        if not input_files:
            raise ValueError("未提供有效的 MapGIS 文件（.wt/.wl/.wp）")
        input_files = sorted(input_files)
        input_dir = os.path.dirname(input_files[0])
    else:
        if not folder:
            raise ValueError("必须提供 folder 或 files 参数")
        input_dir = os.path.abspath(folder.rstrip(os.sep))
        if not os.path.isdir(input_dir):
            raise ValueError(f"不是有效文件夹：{input_dir}")
        input_files = _collect(input_dir, recursive)
    ...
    if output_format == 'shp':
        if not output or not output.strip():
            out_dir = os.path.join(
                ...
                f"{os.path.basename(input_dir)}convert",
            )
        else:
        ...
        if not output or not output.strip():
            gpkg_path = os.path.join(
                os.path.dirname(input_dir),
                f"{os.path.basename(input_dir)}.gpkg",
            )
        else:
    ...
    log.info(f"输入：{input_dir}（{len(input_files)} 个文件）")
    ...
    if output_format == 'shp':
        for i, f in enumerate(input_files, 1):
            ...
            log.info(f"[{i}/{len(input_files)}] {os.path.relpath(f, input_dir)}")
    ...
    if gpkg_engine == 'sqlite3':
        ...
        for i, f in enumerate(input_files, 1):
            ...
            log.info(f"[{i}/{len(input_files)}] {rel}")
